...
buildtest requires the following packages:
...
Installing
^^^^^^^^^^
There are different ways to install buildtest:
...
We have packaged buildtest and published it in PyPi, so you can just install it
with `pip`.

.. code-block:: bash

    pip install -U buildtest
...
Then, install buildtest from TestPypi

.. code-block:: bash

    pip install --index-url https://test.pypi.org/simple/ -U buildtest --pre
...
Then, clone `buildtest Github`_ repository in a folder of your choice:

.. code-block:: bash

    git clone https://github.com/oportoles/testRepo.git
